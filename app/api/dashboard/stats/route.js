  ...
  try {
    ...
    if (rol === 'cuentadante' && documento) {
      ...
      const bienesDisponiblesQuery = `
        SELECT COUNT(*) as total 
        FROM asignaciones 
        WHERE doc_persona = ? AND bloqueado = false
      `;
      ...
      const bienesEnPrestamoQuery = `
        SELECT COUNT(*) as total 
        FROM asignaciones 
        WHERE doc_persona = ? AND bloqueado = true
      `;
    ...
    if (rol === 'usuario' && documento) {
      ...
      const solicitudesCanceladasQuery = `
        SELECT COUNT(*) as total
        FROM solicitudes
        WHERE doc_persona = ? 
          AND estado = 'cancelada'
      `;
      const solicitudesCanceladasResult = await query(solicitudesCanceladasQuery, [documento]);
      const solicitudesCanceladas = parseInt(solicitudesCanceladasResult.rows[0].total);
    ...
    if (rol === 'coordinador' && documento) {
      ...
      const sedeResult = await query(`
        ...
        WHERE rp.doc_persona = ? AND r.nombre = 'coordinador'
      `, [documento]);
    ...
    if (rol === 'vigilante' && documento) {
      ...
      const pendientesQuery = `
        SELECT COUNT(*) as total
        FROM solicitudes
        WHERE estado = 'aprobada' AND sede_id = ?
      `;
      ...
      const enPrestamoQuery = `
        SELECT COUNT(*) as total
        FROM solicitudes
        WHERE estado = 'en_prestamo' AND sede_id = ?
      `;
      ...
      const devueltosQuery = `
        ...
        WHERE s.estado = 'devuelto'
          ...
          AND EXISTS (
            SELECT 1 FROM firma_solicitud fs
            WHERE fs.solicitud_id = s.id 
              AND (fs.rol_usuario = 'vigilante' OR fs.rol_usuario = 'vigilante_entrada')
